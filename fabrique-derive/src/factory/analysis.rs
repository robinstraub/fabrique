...
impl FactoryAnalysis {
    ...
    pub fn analyze(self) -> Result<FactoryAnalysisOutput, Error> {
        let base = Analysis::from(&self.input)?;
    ...
    pub fn _fields(&self) -> Result<&Punctuated<Field, Comma>, Error> {
    ...
    fn relations(&self) -> Result<Vec<Relation>, Error> {
        let fields = Analysis::from(&self.input)?.fields.clone();
        fields
            .iter()
            .filter_map(|field| {
...
mod tests {
    ...
    fn test_a_relation_can_be_created() {
        ...
        let field = &factory._fields().unwrap()[0];
