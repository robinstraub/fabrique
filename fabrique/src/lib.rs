...
//! interact with your database. When using Fabrique, each database table has a
//! corresponding "Model" that is used to interact with that table. In addition
//! to retrieving records from the database table, Fabrique models allow you to
//! insert, update, and delete records from the table as well.
//!
//! ## Fabrique Model Conventions
//!
...
//! tell Fabrique which database table corresponds to our Flight model. By
...
//! Eloquent will assume the `Anvil` model stores records in the `anvils` table,
...
//! you may manually specify the model's table name by defining a table property
//! on the model:
//!
//!```rust,no_run
//! # use fabrique_core::Persistable;
...
//! Otherwise, you must annotate a field with a `fabrique(primary_key)`
...
//! ## Retrieving Models
//!
//! Once you have created a model and **created its corresponding database
//! table**  (using your preferred migration tool or SQL), you are ready to
...
//! `#[fabrique(soft_delete)]` annotation. The attribute type must
...
//! use the [hard_delete][HardDelete::hard_delete] method to permanently remove
