...
- `BelongsTo<User>` for `Order` — exposes the foreign key column for queries
  and factories
- `Joinable<User>` for `Order` — enables
  `Order::query().select().join::<User>()`
- `Joinable<Order>` for `User` — enables
  `User::query().select().join::<Order>()`
...
[query builder](queries.md) filtering orders by the user's primary key.
Fabrique resolves the foreign key column by looking at the `BelongsTo<User>`
trait on `Order`.
...
For a step-by-step guide, see [Handling Multiple belongs_to
Relationships](../guides/multiple-belongs-to-same-model.md).
