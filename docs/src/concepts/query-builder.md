...
The builder's type includes a state parameter: `QueryBuilder<M, S, Joins>`. The
`S` parameter changes as the query is built:
...
Fabrique separates the query builder into two layers, each with distinct
responsibilities:
...
The states mirror SQL clause structure. SQL has rules about ordering —
`ORDER BY` comes after `WHERE`, `OFFSET` requires `LIMIT`. The state machine
encodes these rules.
